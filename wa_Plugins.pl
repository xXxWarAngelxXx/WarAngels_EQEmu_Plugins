######################################################
#:: Usage:			plugin::wa_Decisions(); Then input your own naming structures below. :-)
#:: Created:		12April2020
#:: Version:		wa_200419_01
#:: Author:			WarAngel, modified by Dencelle
#:: Description:	To generate decisions for npcs.
#:: With help from:	
#:: Plugins:		plugin::val('$npc');plugin::MobHealPercentage(5);plugin::RandomRange(1, 100);plugin::wa_KillMode(400, 1200);plugin::wa_ChatMode();
#:: Future Plans:	
#######################################################
...
#:: Instructions:	Below you will see where there are pieces of names (syllables). Dependiing on gender and race_ID insert your own name elements. For example...
#::					"Legolas"...break in down to "Leg ol as". Now below you will see many codes such as... (4 => [1,'Mith Mar Murn Menth Mic','ol ea el','as ere as cha'],)
#::					In that code in the [] ther are three "areas" called arrays. Divided by the "," with elements inside (Mith Mar Murn...) In this case you have the first array with
